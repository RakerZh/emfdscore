...
parser.add_argument('output_metrics', metavar='output_metrics', nargs='+', type=str, default='sentiment',
                    help='eMFD only. Either returns an average sentiment score for each foundation (sentiment) or splits each foundation into a vice/virtue category (vice-virtue). Possible values are: sentiment, vice-virtue')
...
if num_docs == 0:
    print("There are no data in your file. Check if it is loaded")
else:
    if SCORE_METHOD == 'bow':
